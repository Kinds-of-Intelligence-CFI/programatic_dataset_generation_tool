...
def generate_simple_QA(spec: SampleSpec, rng: random.Random) -> Stimulus:
    ...
    if len(spec.demands) > num_operations:
        raise ValueError(
            ...
            f"that has demands {spec.demands}"
        )
...
@validates(name="contains_addition", demand="addition")
def contains_addition(stimulus: Stimulus, spec: SampleSpec) -> None:
    assert "+" in _question_text(stimulus), "addition sample missing '+'"


@validates(name="contains_subtraction", demand="subtraction")
def contains_subtraction(stimulus: Stimulus, spec: SampleSpec) -> None:
    assert "-" in _question_text(stimulus), "subtraction sample missing '-'"


@validates(name="contains_multiplication", demand="multiplication")
def contains_multiplication(stimulus: Stimulus, spec: SampleSpec) -> None:
    assert "*" in _question_text(stimulus), "multiplication sample missing '*'"


@validates(name="contains_division", demand="division")
def contains_division(stimulus: Stimulus, spec: SampleSpec) -> None:
    assert "/" in _question_text(stimulus), "division sample missing '/'"


@validates(name="check_num_ops", demand="*")
...
@validates(name="values_in_range", demand="*")
def values_in_range(stimulus: Stimulus, spec: SampleSpec) -> None:
...
@validates(name="no_unrequested_ops", demand="*")
def no_unrequested_ops(stimulus: Stimulus, spec: SampleSpec) -> None:
    text = _question_text(stimulus)
    for demand, symbol in _SYMBOLS.items():
        if demand not in spec.demands:
            assert symbol not in text, (
                f"sample contains '{symbol}' but '{demand}' is not in spec.demands"
            )


@validates(name="target_matches_question", demand="*")
def target_matches_question(stimulus: Stimulus, spec: SampleSpec) -> None:
...
@validates(name="user_message_present", demand="*")
def user_message_present(stimulus: Stimulus, spec: SampleSpec) -> None:
...
if __name__ == "__main__":
    ...
    # Some (demands, num_operations) pairs are infeasible (more demands
    ...
    run(
        generate_simple_QA, specs, n_reps=2,
        output_dir=out, seed=12345, max_workers=4,
        functional=SampleSpec(params={"min_values": 1}),
    )
